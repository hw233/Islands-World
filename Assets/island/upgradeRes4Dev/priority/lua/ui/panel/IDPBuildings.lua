...
function IDPBuildings.init(csObj)
    ...
    ---@type UIPanel
    uiobjs.Content = uiobjs.scrollView:GetComponent("UIPanel")
    local v4 = IDUtl.getUIContent(csSelf.gameObject)
    local baseClipRegion = uiobjs.Content.baseClipRegion
    baseClipRegion.z = v4.z
    uiobjs.Content.baseClipRegion = baseClipRegion
...
---public 重新设置一次裁剪区域
function IDPBuildings.resetClip()
    if IDPBuildings.isResetClip then
        return
    end
    IDPBuildings.isResetClip = true
    uiobjs.scrollView:ResetPosition()
    uiobjs.Grid:setOldClip(
        uiobjs.Content.clipOffset,
        uiobjs.scrollView.transform.localPosition,
        uiobjs.Grid.transform.localPosition
    )
end

-- 显示，在c#中。show为调用refresh，show和refresh的区别在于，当页面已经显示了的情况，当页面再次出现在最上层时，只会调用refresh
function IDPBuildings.show()
    IDPBuildings.resetClip()
